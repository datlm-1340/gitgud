...
ReviewService.prototype.appendCommentCounts = function () {
  ...
  $.each(files, function (key, item) {
    ...
    if (comments.length) {
      var count = $(
        '<span class="comment-count"><b class="icon-comment">' +
          comments.length +
          "</b></span>"
      );
      $(item).append(count);
...
ReviewService.prototype.appendShowMore = function () {
  ...
  $.each(files, function (key, item) {
    ...
    if (unopenedFile.length > 0) {
      var unopenedFile = $(
        '<span class="unopened-count" title="Large diffs"><b class="icon-unopened">...</b></span>'
      );
...
ReviewService.prototype.reviewDiffs = function (checklist, singleFile) {
  if (singleFile) {
    ...
    var interval = setInterval(function () {
      if (!additions.length) {
        ...
        var file = $("#jk-hierarchy ").find(
          "[data-file-id=" + singleFile + "]"
        );

        $(file).find(".unopened-count").hide();
        review(checklist, file, additions, true);
    ...
    $.each(files, function (key, file) {
      ...
      review(checklist, file, additions);
    });
  ...
  function appendWarningCounts(count, file) {
    if (count > 0) {
      var count = $(
        '<span class="warning-count"><b class="icon-warning">' +
          count +
          "</b></span>"
      );
  ...
  function appendDangerCounts(count, file) {
    if (count > 0) {
      var count = $(
        '<span class="danger-count"><b class="icon-danger">' +
          count +
          "</b></span>"
      );
  ...
  function appendIndicators(report, addition, isSingleFile) {
    if (!$(addition).hasClass("has-warnings"))
      appendWarningIndicators(report, addition);
    if (!$(addition).hasClass("has-dangers"))
  ...
  function filterByFileType(checklist, fileType) {
    return checklist.reduce(function (arr, item) {
      if (item.file.split("_").includes(fileType)) arr.push(item);
  ...
  function review(checklist, file, additions, isSingleFile) {
    ...
    $.each(additions, function (index, addition) {
      var unformattedLine = $(addition)
        .find(".blob-code-inner.blob-code-marker")
        .html();
